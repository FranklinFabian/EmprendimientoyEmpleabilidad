...
window.loadWindow = function(id, type) {
    var pid = $('#formacion_pid').val();
    $.get(HOST_URL + '/empleabilidad/curriculo/formacion/' + id + '/' + type + '/' + pid + '/edit', function (data) {
        $("#modal-content-formacion").html(data);
        $("#form_modal_formacion").modal("show");
    });
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/empleabilidad/curriculo/formacion/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    ...
    var initTable1 = function() {
        window.table = $('#formacion_datatable').DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: HOST_URL + '/empleabilidad/curriculo/formacion_datatable/' + pid,
            },
            columns: [

                { data: 'id', name: 'f.id', title: 'Acciones' },
                { data: 'descripcion', name: 'f.descripcion', title: 'Descripción' },
            ],
            columnDefs: [
                {
                    targets: 0,
