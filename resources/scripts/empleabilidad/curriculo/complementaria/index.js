...
window.loadWindow = function(id, type) {
    var pid = $('#complementaria_pid').val();
    $.get(HOST_URL + '/empleabilidad/curriculo/complementaria/' + id + '/' + type + '/' + pid + '/edit', function (data) {
        $("#modal-content-complementaria").html(data);
        $("#form_modal_complementaria").modal("show");
    });
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/empleabilidad/curriculo/complementaria/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    ...
    var initTable1 = function() {
        window.table = $('#complementaria_datatable').DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: HOST_URL + '/empleabilidad/curriculo/complementaria_datatable/' + pid,
            },
            columns: [

                { data: 'id', name: 'c.id', title: 'Acciones' },
                { data: 'descripcion', name: 'c.descripcion', title: 'Descripción' },
            ],
            columnDefs: [
                {
                    targets: 0,
