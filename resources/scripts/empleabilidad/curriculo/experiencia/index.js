...
window.loadWindow = function(id, type) {
    var pid = $('#experiencia_pid').val();
    $.get(HOST_URL + '/empleabilidad/curriculo/experiencia/' + id + '/' + type + '/' + pid + '/edit', function (data) {
        $("#modal-content-experiencia").html(data);
        $("#form_modal_experiencia").modal("show");
    });
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/empleabilidad/curriculo/experiencia/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    ...
    var initTable1 = function() {
        window.table = $('#experiencia_datatable').DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: HOST_URL + '/empleabilidad/curriculo/experiencia_datatable/' + pid,
            },
            columns: [

                { data: 'id', name: 'e.id', title: 'Acciones' },
