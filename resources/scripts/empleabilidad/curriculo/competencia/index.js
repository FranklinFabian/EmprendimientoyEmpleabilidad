...
window.loadWindow = function(id, type) {
    var pid = $('#competencia_pid').val();
    $.get(HOST_URL + '/empleabilidad/curriculo/competencia/' + id + '/' + type + '/' + pid + '/edit', function (data) {
        $("#modal-content-competencia").html(data);
        $("#form_modal_competencia").modal("show");
    });
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/empleabilidad/curriculo/competencia/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    ...
    var initTable1 = function() {
        window.table = $('#competencia_datatable').DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: HOST_URL + '/empleabilidad/curriculo/competencia_datatable/' + pid,
            },
            columns: [
