...
window.loadWindow = function(id, tipo) {
    window.location.href = HOST_URL + '/empleabilidad/ficha/tab/' + id + '/' + tipo ;
};

window.itemPdf = function(id) {
    window.open('/empleabilidad/ficha/item/ficha_pdf/' + id, '_blank');
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/empleabilidad/ficha/item/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    var initTable1 = function() {
        window.table = $('#kt_datatable').DataTable({
            ...
            ajax: {
                url: HOST_URL + '/empleabilidad/ficha/item',
                type: 'GET'
            },
            columns: [
                { data: 'id', name: 'fs.id', title: 'Acciones' },
                { data: 'fecha', name: 'fs.fecha', title: 'Fecha' },
