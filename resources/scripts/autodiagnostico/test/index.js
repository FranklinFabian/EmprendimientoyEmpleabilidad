...
window.loadWindow = function(id, tipo) {
    window.location.href = HOST_URL + '/autodiagnostico/test/tab/' + id + '/' + tipo ;
};

window.itemPdf = function(id) {
    window.open('/autodiagnostico/test/item/ficha_pdf/' + id, '_blank');
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/autodiagnostico/test/item/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    var initTable1 = function() {
        window.table = $('#kt_datatable').DataTable({
            ...
            ajax: {
                url: HOST_URL + '/autodiagnostico/test/item',
                type: 'GET'
            },
            columns: [
                { data: 'id', name: 'a.id', title: 'Acciones' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        boton =
                            '<div align="center">' +
                                ...
                                '<a href="javascript:itemDelete(\''+data+'\');" class="btn btn-outline-danger btn-sm  mr-2 mb-2">'+
