...
window.loadWindow = function(id, tipo) {
    window.location.href = HOST_URL + '/emprendedurismo/canva/tab/' + id + '/' + tipo ;
};

window.itemPdf = function(id) {
    window.open('/emprendedurismo/canva/item/ficha_pdf/' + id, '_blank');
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/emprendedurismo/canva/item/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    var initTable1 = function() {
        window.table = $('#kt_datatable').DataTable({
            ...
            ajax: {
                url: HOST_URL + '/emprendedurismo/canva/item',
                type: 'GET'
            },
            columns: [
