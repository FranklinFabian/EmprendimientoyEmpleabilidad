...
window.loadWindow = function(id, tipo) {
    window.location.href = HOST_URL + '/emprendedurismo/evaluacion/tab/' + id + '/' + tipo ;
};

window.itemPdf = function(id) {
    window.open('/emprendedurismo/evaluacion/item/ficha_pdf/' + id, '_blank');
};
...
function itemDeleteAction(id){
    $.ajax({
        url : HOST_URL + '/emprendedurismo/evaluacion/item/' + id,
        type: 'DELETE'
...
var KTDatatablesDataSourceAjaxServer = function() {
    var initTable1 = function() {
        window.table = $('#kt_datatable').DataTable({
            ...
            ajax: {
                url: HOST_URL + '/emprendedurismo/evaluacion/item',
                type: 'GET'
            },
            columns: [
