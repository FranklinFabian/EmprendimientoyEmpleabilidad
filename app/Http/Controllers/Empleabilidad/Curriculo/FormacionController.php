...
use App\Models\Empleabilidad\Curriculo\Formacion;
...
class FormacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('pages.empleabilidad.curriculo.formacion.index')->with('data', $data);
    }
    ...
    public function edit($id, $type, $pid)
    {
        ...
        if($type == 'update'){
            $data->id = $id;
            $data->tipo = $type;
            $data->item = Formacion::find($id);
        }else{
            $data->tipo = 'new';
        }
        $data->pid = $pid;
        return view('pages.empleabilidad.curriculo.formacion.modal')->with('data', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            Formacion::updateOrCreate(['id' => $request->id], $data);
            $res->id = 1;
        } catch (\Exception $e) {
            $res->id = 0;
        }
